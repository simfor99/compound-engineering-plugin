...
- Do not treat "true" as sufficient for "actionable." Before emitting a finding, ask whether the smallest adequate fix improves the current document's target outcome more than it increases carrying cost. If the concern only matters under a different deployment mode, later maturity level, larger team, public exposure, production scale, regulated context, or future automation that the document does not claim, suppress it or emit at most an advisory `confidence: 50` observation when the context caveat itself is useful.
...
- **Context-mismatched hardening or process** — controls, gates, abstractions, governance, evidence packages, rollout plans, or security hardening that would be appropriate for a different target context but are not needed for the document's stated use case. Examples: production/public-server controls for a local-only development tool, team-governance rituals for a solo-maintained scratchpad, durable audit evidence for a throwaway exploration, or reusable abstraction work for a one-off migration. Surface as FYI only when the document is likely to cross that boundary soon and the caveat helps the reader.
