...
1. **Start `/ce-work`** (recommended) - Implement this plan now via the `ce-work` skill, which runs the work and owns the implementation tail. Implementation-ready code plans only.
2. **Give me the `/goal` prompt** - Print the plan's Goal Launch Block as a copyable top-level `/goal` prompt for the user to run; `ce-work` does NOT also run. Implementation-ready code plans only. (Options 1 and 2 are alternative executors — pick one.)
