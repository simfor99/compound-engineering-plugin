...
1. **Start `/ce-work`** (recommended) - Implement this plan now, in this session — the assistant builds it via the `ce-work` skill. Implementation-ready code plans only.
2. **Give me the `/goal` prompt** - Hand the user a copyable `/goal` prompt to run themselves instead of building now; the assistant does not start building. The alternative to option 1, not an add-on — pick one. Implementation-ready code plans only, and only on a host with a top-level `/goal` command (Claude Code and Codex today).
...
- **Give me the `/goal` prompt** — Offered on the implementation-ready-code gate **and only on a host with a top-level `/goal` command (Claude Code and Codex today)**; omit it elsewhere. Print the plan's standalone Goal Launch Block as a copyable top-level `/goal` prompt and tell the user to paste it at the start of a message. **Do not invoke `ce-work` and do not start execution** — the user's `/goal` is the executor and owns the tail; running both would double-execute the plan. A skill cannot issue `/goal` itself, so handing over the prompt is the complete action; then return to the menu.
- **Review remaining findings** — Re-invoke the `ce-doc-review` skill on the plan path **without** `mode:headless` so the interactive routing question and walkthrough fire. After it returns, re-render this menu with refreshed counts so the user can pick a next-stage action.
...
If the user types free-form prompts targeting the findings (e.g., "review", "walk through", "deep review"), route as if they picked `Review remaining findings` — fire the skill rather than looping back to the menu. For other free-text revisions, accept the input and loop back to this menu after applying the revision.
