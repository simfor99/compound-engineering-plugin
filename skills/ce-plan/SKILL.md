...
- **Create a `/goal` prompt** — Offered on the implementation-ready-code gate, and only where the host has goal mode (callable tool or user-typed `/goal`). **`ce-work` does not also run.** If a callable goal tool is available (Codex `create_goal`), call it with the plan's Goal Launch Block objective — the session works toward the DoD; do not call `update_goal` (the goal session completes itself). Otherwise (user-typed `/goal` only, e.g. Claude Code), print the copyable Goal Launch Block prompt for the user to paste, then return to the menu.
