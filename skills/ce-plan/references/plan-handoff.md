...
**Options:**
1. **Start `/ce-work`** (recommended) - Implement this plan now, in this session — the assistant builds it via the `ce-work` skill. Show only for `artifact_readiness: implementation-ready` plus `execution: code`; universal-planning, answer-seeking, approach-plan, and requirements-only artifacts keep their own handoff/checkpoint behavior.
2. **Give me the `/goal` prompt** - Hand the user a copyable `/goal` prompt to run themselves instead of building now; their `/goal` drives implementation to the Definition of Done, and the assistant does not start building. This is the alternative to option 1, not an add-on — pick one. Show only when (a) the artifact is `artifact_readiness: implementation-ready` plus `execution: code` AND (b) the current host has a top-level `/goal` command — Claude Code and Codex today; omit it on hosts without one (a skill cannot invoke `/goal` itself, so on those hosts there is nothing useful to hand over). The same prompt also lives in the plan's Goal Launch Block section, so this is a convenience copy.
3. **Review remaining findings** - The auto-pass already applied the safe, mechanical fixes (typos, wrong counts, stale references). This walks you through what it left for your judgment — proposed edits that would change the plan's meaning or scope (confirm or skip each) and open judgment calls with no single right answer (decide, or defer them into Open Questions).
...
**Option 5 format-keyed label.** Under exclusive output mode, the plan exists as exactly one artifact — `.md` or `.html`, never both. Render the option 5 label matching the produced format. Proof ingests the `.md` source, so it does not apply to HTML runs; the browser option opens the local `.html` file directly. Implementation handoff (options 1 and 2) remains available in both modes only when the artifact is implementation-ready code — `ce-work` reads either format, and the Goal Launch Block prompt is emitted regardless of format (see the ce-work skill's plan-input handling).
...
- **Start `/ce-work`** -> Classify the artifact first. If it is not `artifact_readiness: implementation-ready` plus `execution: code`, do not execute it; route requirements-only artifacts back to `ce-plan` enrichment and non-code artifacts to their own workflow. If it is executable, invoke the `ce-work` skill via the platform's skill-invocation primitive, passing the plan path as the skill argument; `ce-work` then owns engine selection (inline/subagent vs goal-mode vs dynamic-workflow) and the implementation tail. If no skill-invocation primitive exists on this host, print the `ce-work` fallback prompt for the user to run; in that prompt, tell the executor to read Reader Index, Goal Capsule, Verification Contract, Definition of Done, and active U-IDs rather than the whole document first. Do not merely tell the user to type `/ce-work` when a skill invocation primitive is available.
- **Give me the `/goal` prompt** -> Print the plan's standalone Goal Launch Block (the top-level `/goal` prompt) as a single copyable block and tell the user to paste it at the start of a message. **Do not invoke `ce-work`, and do not start any execution** — the user's top-level `/goal` is the executor and owns the implementation tail; also running `ce-work` would double-execute the plan and split tail ownership. A skill cannot issue `/goal` itself (it is recognized only at the start of a user message on Claude Code, and is composer/user-driven on Codex), so handing over the copyable prompt is the complete action. After printing, return to the options. Render only for implementation-ready code plans **and only on a host with a top-level `/goal` command (Claude Code and Codex today)** — omit the option entirely on hosts without one.
- **Review remaining findings** -> Re-invoke the `ce-doc-review` skill on the plan path **without** `mode:headless` so the interactive routing question and walkthrough fire. The headless pass already applied `safe_auto` fixes and recorded its findings in the session, so the interactive pass picks up where headless stopped — its R29 suppression rule prevents prior-round Skipped/Deferred entries from re-raising. After it returns, re-render this menu with the refreshed counts so the user can pick what to do next.
...
- **Free-form prompts that target the findings** (e.g., the user types "review", "walk through", "deep review" instead of picking a numbered option) -> route as if they had picked `Review remaining findings`. Do not loop back to the menu without firing the review. **Exception:** when the envelope carries `skipped_reason: output_format_html`, do not fire ce-doc-review — instead, reply once with `ce-doc-review is markdown-only today; the HTML plan can't be reviewed without HTML-aware mutation support. Switch to /ce-plan output:md to regenerate as markdown if you want a review pass.` and loop back to the menu.
