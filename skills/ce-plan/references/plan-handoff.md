...
**Options:**
1. **Start `/ce-work`** (recommended) - Begin implementing this plan in the current session; when goal-mode is supported but not callable from this skill, show the copyable Goal Launch Block instead. Show only for `artifact_readiness: implementation-ready` plus `execution: code`; universal-planning, answer-seeking, approach-plan, and requirements-only artifacts keep their own handoff/checkpoint behavior.
...
**Option 4 format-keyed label.** Under exclusive output mode, the plan exists as exactly one artifact — `.md` or `.html`, never both. Render the option 4 label matching the produced format. Proof ingests the `.md` source, so it does not apply to HTML runs; the browser option opens the local `.html` file directly. Implementation handoff remains available in both modes only when the artifact is implementation-ready code — `ce-work` reads either format (see the ce-work skill's plan-input handling).
...
- **Start `/ce-work`** -> Invoke the `ce-work` skill via the platform's skill-invocation primitive when available, passing the plan path as the skill argument; first classify the artifact. If it is not `artifact_readiness: implementation-ready` plus `execution: code`, do not execute it; route requirements-only artifacts back to `ce-plan` enrichment and non-code artifacts to their own workflow. If it is executable, prefer the host's best implementation engine:
  - When a callable skill invocation is available, invoke `ce-work` with the plan path.
  - When the host supports top-level goal mode but a skill cannot invoke it directly, print the plan's Goal Launch Block as a copyable prompt and say that the user should run it at top level.
  - When dynamic workflow / `ultracode:` is appropriate for large independent U-ID fan-out, print that block only when the plan's Goal Launch Block includes it.
  In every case, the launcher prompt must tell the executor to read Reader Index, Goal Capsule, Verification Contract, Definition of Done, and active U-IDs rather than the whole document first. Do not merely tell the user to type `/ce-work` when a skill invocation primitive is available.
