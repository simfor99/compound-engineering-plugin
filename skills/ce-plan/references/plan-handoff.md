...
- **Start `/ce-work`** -> Invoke the `ce-work` skill via the platform's skill-invocation primitive (`Skill` in Claude Code, `Skill` in Codex, the equivalent on Gemini/Pi), passing the explicit plan path as the skill argument. This path may later move under `docs/plans/_archive/` after successful execution; downstream review/commit skills must accept the explicit archived path instead of relying on active-root discovery. Do not merely tell the user to type `/ce-work` — fire the invocation now so the plan executes in this session.
