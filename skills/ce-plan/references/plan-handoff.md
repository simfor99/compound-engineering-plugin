...
**Options:**
1. **Start `/ce-work`** (recommended) - Begin implementing this plan now via the `ce-work` skill (it runs the work inline/with subagents and owns the implementation tail). Show only for `artifact_readiness: implementation-ready` plus `execution: code`; universal-planning, answer-seeking, approach-plan, and requirements-only artifacts keep their own handoff/checkpoint behavior.
2. **Give me the `/goal` prompt** - Print the plan's Goal Launch Block as a copyable prompt to run at the **top level** of a message; the user's `/goal` then drives implementation to the Definition of Done. **`ce-work` does NOT also run** — this is the alternative executor to option 1, not an add-on. A skill cannot invoke `/goal` itself (it is recognized only at the start of a user message on Claude Code, and is composer/user-driven on Codex), so this option hands the user a prompt rather than starting anything. Show only for `artifact_readiness: implementation-ready` plus `execution: code`. The prompt also lives in the plan's Goal Launch Block section, so this is a convenience copy, not the only access path.
