...
- **Context-mismatched hardening or process.** Do not turn production-only, enterprise-only, or governance-heavy concerns into findings for local tools, fixtures, tests, docs, prototypes, one-off migrations, or development-only wiring unless the diff creates a reachable path into production behavior, user data, credentials, billing, deploys, or shared runtime contracts.
...
**Relevance and smallest-fix check.** A finding is not actionable merely because it is true. Before emitting, ask whether the smallest defensible fix improves user, caller, operator, data, or contract outcomes more than the extra code/process/testing surface it adds. Keep concrete reachable bugs, vulnerabilities, data-loss paths, auth failures, crashes, compile/build breaks, and contract violations. Suppress or downgrade advice whose fix would add policy, abstraction, framework, persistence, config, monitoring, retry, cache, permission, validation, or process surface without a current failure path. If the finding survives, make `suggested_fix` the smallest adequate change: a local guard, deletion, focused test, contract correction, or short documentation note rather than a broad redesign.
...
- **Propose a `suggested_fix` whenever any defensible code change is reachable from the diff and surrounding code.** This is the persona's commitment that "I, the reviewer with the diff and evidence in front of me, can articulate what the fix looks like." The suggested fix becomes the authoritative signal that downstream surfaces use to decide whether the agent can act on the finding. Three rules:
  ...
  - **Smallest adequate change:** propose the least new surface that closes the observed issue. Prefer deleting unnecessary code, adding a local guard, tightening an existing contract, or adding one targeted test over introducing a new abstraction, policy layer, framework, queue, cache, monitor, or process.
